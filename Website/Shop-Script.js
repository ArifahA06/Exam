...
function displayProductDetail() {
    ...
    let selectorColor = productData.colors[0];
    let selectorSize = selectorColor.sizes[0];

    function updateProductDisplay(colorData) {
        if (!colorData.sizes.includes(selectorSize)) {
            selectorSize = colorData.sizes[0];
        }
        mainImageContainer.innerHTML = `<img src="${colorData.mainImage}">`;

        thumbnailContainer.innerHTML = "";
        const allImages = [colorData.mainImage].concat(colorData.additionalImages.slice(0, 3));
        ...
        productData.colors.forEach(color => {
            ...
            if (color.name === selectorColor.name) img.classList.add("selected");
        ...
        colorData.sizes.forEach(size => {
            ...
            if (size === selectorSize) sizeBtn.classList.add("selected");
    ...
    updateProductDisplay(selectorColor);

    addToCartBtn.addEventListener("click", () => {
        addToCart(productData, selectorColor, selectorSize);
    });
}
...
function addToCart(product, color, size) {
    ...
    const existingItem = cart.find(item => item.productId === product.id && item.color === color.name && item.size === size);

    if (existingItem) {
        existingItem.quantity += parseInt(document.querySelector("#quantity").value);
    } else {
        cart.push({
            id: product.id,
            title: product.title,
